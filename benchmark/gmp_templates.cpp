...
u64 gmp_sub_impl  (const config&, int limbs);
...
template <int limbs> u64 gmp_sub  (const config &conf) { return gmp_sub_impl  (conf, limbs); }
...
template u64 gmp_sub<LIMBS>(const config&);
